...
const mockAddresses = [
	{ id: '1', code: 'YAO-B123', city: 'Yaound̩', quarter: 'Bastos, Rue 1.024', resident: 'Jean-Pierre Nkou', status: 'Valid̩', phone: '+237 6xx xx xx xx', buildingImg: 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80' },
	{ id: '2', code: 'DLA-A045', city: 'Douala', quarter: 'Akwa, Bd de la Libert̩', resident: 'Marie-Th̩r̬se Ebollo', status: 'En attente', phone: '+237 6xx xx xx xx' },
	{ id: '3', code: 'YAO-K089', city: 'Yaound̩', quarter: 'Kotto, Villa 12', resident: 'Oumarou Bello', status: 'Signal̩', phone: '+237 6xx xx xx xx' },
	{ id: '4', code: 'DSG-X992', city: 'Dschang', quarter: 'Centre Ville, Secteur A', resident: 'Lucas Fotso', status: 'Valid̩', phone: '+237 6xx xx xx xx' },
];
...
const getStatusStyles = (status: string) => {
	...
	if (s.includes('valid̩')) return 'bg-[#81C784] text-white';
	if (s.includes('signal̩')) return 'bg-rose-100 text-rose-700';
...
<template>
	<div class="h-full flex flex-col xl:flex-row gap-6">
		...
		<!-- COLONNE GAUCHE (Filtres Avanc̩s) -->
		<div class="w-full xl:w-[280px] shrink-0 flex flex-col gap-6">
			<div>
				<h1 class="text-2xl font-black text-[#155dfc] mb-1">R̩pertoire des Adresses</h1>
				<p class="text-xs text-gray-500 font-medium">G̩rez et validez le registre national des adresses FindMe.</p>
			</div>

			<div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex-1">
				<div class="flex items-center gap-2 mb-6">
					<Filter class="w-5 h-5 text-gray-400" />
					<h3 class="font-black text-gray-800">Filtres Avanc̩s</h3>
				</div>
					...
					<div>
						...
						<div class="space-y-3">
							<label class="flex items-center gap-3 cursor-pointer">
								...
								<span class="text-sm text-gray-700 font-semibold">Valid̩</span>
							</label>
							<label class="flex items-center gap-3 cursor-pointer">
								<div class="w-5 h-5 rounded-full bg-[#81C784] flex items-center justify-center">
									<div class="w-2 h-2 bg-white rounded-full"></div>
							...
							<label class="flex items-center gap-3 cursor-pointer">
								...
								<span class="text-sm text-gray-700 font-semibold">Signal̩</span>
							</label>
						</div>
					</div>
				</div>
		...
		<div class="flex-1 flex flex-col gap-6 overflow-hidden">
			...
			<div class="bg-white rounded-3xl border border-gray-100 shadow-sm flex flex-col flex-1 overflow-hidden">
				<div class="p-4 border-b border-gray-100 flex items-center justify-between">
					<p class="text-sm font-bold text-gray-800">{{ filteredAddresses.length }} Adresses trouv̩es</p>
					...
					<table class="w-full text-left border-collapse min-w-[500px]">
						<thead>
							<tr class="bg-white text-[10px] font-black text-gray-400 uppercase tracking-wider border-b border-gray-100">
								...
								<th class="px-5 py-4">R̩sident</th>
						...
						<tbody class="divide-y divide-gray-50">
							...
							>
								...
								<td class="px-5 py-4 text-right">
									<!-- Actions invisibles/vides sur la maquette mais pr̩vues -->
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6 h-[160px] shrink-0">
				...
				<div class="bg-[#0A0F2C] rounded-3xl p-6 text-white relative overflow-hidden shadow-lg">
					...
					<p class="text-[10px] italic text-[#8C9EFF] mt-3 relative z-10">"L'objectif de couverture urbaine est de 85% d'ici la fin de l'ann̩e."</p>
				</div>
				<!-- Aide -->
		...
		<div class="w-full xl:w-[320px] shrink-0 flex flex-col gap-4">
			<div v-if="selectedAddress" class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full">
				<div class="p-4 border-b border-gray-100 flex items-center justify-between">
					<h3 class="font-black text-gray-800">D̩tails de l'Adresse</h3>
				...
				<div class="overflow-y-auto flex-1 p-5 space-y-6" style="scrollbar-width: none;">
					...
					<div class="p-4 border border-gray-100 rounded-2xl flex items-center gap-3 bg-[#F8F9FB]">
						...
						<div>
							<p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Propri̩taire / R̩sident</p>
					...
					<div>
						...
						<div class="space-y-4 relative before:absolute before:inset-y-0 before:left-[5px] before:w-px before:bg-gray-200">
							<div class="relative pl-6">
								...
								<p class="text-xs font-bold text-gray-800">Valid̩ par Admin 02</p>
			...
			<div v-else class="bg-white rounded-3xl border border-gray-100 shadow-sm flex items-center justify-center h-full p-8 text-center text-gray-400">
				S̩lectionnez une adresse pour voir les d̩tails.
			</div>
		</div>
