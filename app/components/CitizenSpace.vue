...
<template>
  <section v-if="currentView === 'citizen-space' && currentUser" class="bg-[#FAF8FD] dark:bg-[#0E111F] border-b-4 border-[#1A237E] py-16 px-6 min-h-[70vh]" id="mon-espace-citoyen-section">
    <div class="max-w-6xl mx-auto space-y-10">
      
      <!-- Fil d'Ariane pour la navigation arrière -->
      <div class="flex items-center space-x-2 text-sm text-[#1A237E]/70 dark:text-slate-300 mb-2 font-semibold" id="citizen-breadcrumbs">
        <ButtonUI 
          variant="outline" 
          @click="handleGoHome"
          id="go-home-btn"
      ...
      <div class="bg-white dark:bg-slate-900 border-4 border-[#1A237E] dark:border-[#2E7D32] rounded-[32px] p-6 md:p-8 shadow-[8px_8px_0px_0px_#1A237E] dark:shadow-[8px_8px_0px_0px_#2E7D32] flex flex-col md:flex-row items-start md:items-center justify-between gap-6" id="espace-header-card">
        <div class="space-y-2">
      ...
      <div class="space-y-6" id="user-addresses-flow">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b-2 border-[#1A237E]/10 pb-3 gap-4" id="addresses-flow-header-wrapper">
          ...
          <div class="flex items-center bg-white dark:bg-slate-950 border-2 border-[#1A237E] dark:border-slate-800 rounded-xl p-1 shadow-[2px_2px_0px_0px_#1A237E]" id="view-mode-selector-tabs">
            <button
              type="button"
        ...
        <div v-if="citizenSpaceTab === 'list'" class="grid grid-cols-1 md:grid-cols-2 gap-8" id="addresses-cards-grid">
          <div 
          ...
          >
            <div class="absolute top-0 right-0 bg-[#1A237E] text-white font-mono text-[9px] px-4 py-1.5 rounded-bl-2xl font-black uppercase tracking-widest">
              ID: #{addressesList.length - idx}
            </div>

            <div @click="openAddressDetails(idx)" class="space-y-4 cursor-pointer hover:opacity-95 transition-opacity group">
              ...
              <div class="space-y-0.5">
                <span class="text-[8px] font-black uppercase text-[#2E7D32] tracking-wider block">LOCALISATION</span>
                <h4 class="text-lg font-black text-[#1A237E] dark:text-white leading-tight">
                  {{ addr.housePlateNumber }}, {{ addr.arrondissement || 'Yaoundé II' }} • {{ addr.neighborhood }}
                </h4>
                <span class="text-[11px] font-bold text-[#1A237E]/75 dark:text-gray-300">
                  {{ addr.city }}, Cameroun {{ addr.postalCode ? `(CP: ${addr.postalCode})` : '' }}
                </span>
              </div>

              <!-- Aperçu Photo -->
              <div class="border-2 border-[#1A237E]/10 rounded-2xl overflow-hidden h-40 bg-[#F5F2FB] dark:bg-slate-950 relative" :id="`addr-photo-prev-${idx}`">
                <template v-if="addr.photoRaw">
                  ...
                  <div class="absolute bottom-2 left-2 right-2 bg-black/80 backdrop-blur-sm text-white px-3 py-1.5 rounded-xl text-[9px] leading-relaxed font-semibold flex items-center justify-between border border-white/5 shadow-md">
                    ...
                    <span v-else class="font-mono text-[8.5px] bg-[#2E7D32] px-2 py-0.5 rounded text-white font-black whitespace-nowrap shrink-0">210 KB</span>
                  </div>
                </template>
                <div v-else class="w-full h-full flex flex-col items-center justify-center text-center p-4 text-[#1A237E]/40 dark:text-slate-400 space-y-1">
              ...
              <div class="grid grid-cols-2 gap-2 text-center" id="addr-gps-row">
                <div class="bg-[#1A237E]/5 dark:bg-slate-800 px-2.5 py-2 rounded-xl border border-[#1A237E]/10 dark:border-slate-800">
                  <span class="text-[7px] uppercase font-black text-[#1A237E]/50 dark:text-gray-400 block">Latitude</span>
                  <span class="text-[11px] font-mono font-bold text-[#1A237E] dark:text-sky-300">{{ addr.coordinates.lat }}° N</span>
                </div>
                <div class="bg-[#1A237E]/5 dark:bg-slate-800 px-2.5 py-2 rounded-xl border border-[#1A237E]/10 dark:border-slate-800">
                  <span class="text-[7px] uppercase font-black text-[#1A237E]/50 dark:text-gray-400 block">Longitude</span>
              ...
              <div class="bg-[#2E7D32]/15 border-2 border-[#2E7D32]/35 p-3.5 rounded-2xl text-center">
                <span class="text-[8.5px] font-black text-[#2E7D32] uppercase tracking-widest block leading-none mb-1">CODE D'ADRESSAGE UNIQUE</span>
                <span class="font-mono text-base md:text-lg font-black text-[#1A237E] dark:text-white block tracking-widest uppercase">
                  {{ addr.addressCode }}
                </span>
              </div>
            </div>
            ...
            <!-- Actions par plaque -->
            <div class="mt-5 pt-4 border-t-2 border-[#1A237E]/10 dark:border-slate-800 flex items-center justify-between gap-2 flex-wrap">
              <button
                type="button"
