...
<script setup lang="ts">
		...
		const latBase =
			localAddress.value.city === "Douala"
				...
				: localAddress.value.city === "Yaound̩"
					...
					: 5.4778;
		const top =
			((localAddress.value.coordinates.lat - latBase) / 0.02 + 0.5) * 100;
		...
		const lngBase =
			localAddress.value.city === "Douala"
				...
				: localAddress.value.city === "Yaound̩"
					...
					: 10.4178;
		const left =
			((localAddress.value.coordinates.lng - lngBase) / 0.02 + 0.5) * 100;
	...
	const adjustCoordinates = (direction: "up" | "down" | "left" | "right") => {
		...
		localAddress.value = {
			...
			coordinates: {
				lat: parseFloat(
					Math.max(1.5, Math.min(14.0, currentCoords.lat)).toFixed(5),
				),
				lng: parseFloat(
					Math.max(8.0, Math.min(17.0, currentCoords.lng)).toFixed(5),
				),
			},
		};
	};
