...
import { PlusCircle, X, CheckSquare, ShieldCheck, Sparkles } from 'lucide-vue-next'
...
const { validateCameroonHouseNumber, validateCameroonPostalCode } = useValidation()

const formFullName = ref('')
const formPhone = ref('')
const formCity = ref<'Yaoundé' | 'Douala' | 'Bafoussam' | 'Garoua' | 'Kribi'>('Yaoundé')
...
const formPhoto = ref<{ raw: string; stats: { compressed: string; ratio: string } } | null>(null)
...
const handleFormSubmit = (e: Event) => {
  ...
  errors.value.fullName = !formFullName.value.trim() ? "Le nom complet est obligatoire." : ""
  errors.value.phone = !formPhone.value.trim() ? "Le téléphone de raccord est obligatoire." : ""
  errors.value.quartier = !formQuartier.value.trim() ? "Le quartier de raccord est obligatoire." : ""
  ...
  if (errors.value.fullName || errors.value.phone || errors.value.quartier || errors.value.houseNumber || errors.value.postalCode) {
    return
  }
  ...
  setTimeout(() => {
    const cityCompact = formCity.value === 'Douala' ? 'DLA' : 
                        formCity.value === 'Yaoundé' ? 'YDE' :
                        formCity.value === 'Bafoussam' ? 'BFM' :
                        formCity.value === 'Garoua' ? 'GAR' : 'KRI'
    const block = formQuartier.value.substring(0, 3).toUpperCase().replace(/\s/g, '')
    ...
    const newAddr = {
      ...
      coordinates: {
        lat: parseFloat(formLat.value) || 3.8480,
        lng: parseFloat(formLng.value) || 11.5021
      },
      addressCode: finalCode,
      photoRaw: formPhoto.value?.raw || '/assets/images/cameroon_house_address_1780109511639.png',
      photoStats: {
...
<template>
  ...
  >
    <div class="relative w-full max-w-4xl bg-white border-4 border-[#1A237E] rounded-[32px] shadow-2xl p-6 md:p-8 space-y-6 max-h-[92vh] overflow-y-auto">
      ...
      <div class="text-left space-y-2">
        <h2 id="address-form-modal-title" class="text-2xl md:text-3xl font-black text-[#1A237E] flex items-center space-x-2">
          <PlusCircle class="w-7 h-7 text-[#2E7D32]" />
          <span>Immatriculer mon Adresse</span>
        </h2>
        <p class="text-[11px] font-bold text-[#1A237E]/60 flex items-center space-x-1.5 uppercase tracking-wider">
      ...
      <!-- Corps du formulaire -->
      <form @submit="handleFormSubmit" class="space-y-6" id="creation-address-core-form">
        <FormUserInfoFields 
        ...
        <FormPhotoCapture 
          @photo-loaded="(img) => formPhoto = img"
        />

        <!-- Soumission -->
        <div class="pt-4 flex justify-end space-x-3 border-t border-[#1A237E]/10" id="form-submit-footer">
          <ButtonUI 
            type="button" 
            variant="outline"
            @click="emit('close')"
