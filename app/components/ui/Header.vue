...
<template>
  <header class="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b-2 border-[#1A237E]/10" id="header-container">
    <div class="max-w-7xl mx-auto px-4 md:px-8 py-3.5 flex items-center justify-between">
      ...
      <div 
        ...
        id="logo-wrapper"
      >
        <FindMeLogo size="130" id="logo-badge" />
      </div>

      <!-- Navigation Desktop -->
      <nav class="hidden lg:flex items-center space-x-8 text-[#1A237E] font-medium text-sm" id="main-navigation" role="navigation" aria-label="Menu principal">
        <button 
      ...
      <div class="hidden lg:flex items-center space-x-3.5" id="header-actions">
        ...
        <div v-if="currentUser" class="flex items-center bg-[#2E7D32]/10 border-2 border-[#2E7D32]/30 rounded-2xl p-0.5 pr-3.5" id="header-user-status">
          <div 
          ...
          >
            <div class="w-8 h-8 rounded-full bg-[#2E7D32] text-white flex items-center justify-center font-black text-xs border border-[#1A237E]/20 shadow-sm uppercase shrink-0">
              {{ currentUser.username ? currentUser.username.substring(0, 2) : currentUser.email.substring(0, 2) }}
            </div>
            <div class="text-left">
              <span class="text-[9px] text-[#2E7D32] uppercase font-black tracking-widest block leading-3">Bienvenue</span>
              <span class="text-xs font-black text-[#1A237E] block truncate max-w-[125px]" :title="currentUser.username || currentUser.email">
    ...
    <div v-if="menuOpen" class="lg:hidden bg-white border-t-2 border-[#1A237E]/10 px-6 py-5 space-y-4" id="mobile-menu-drawer" role="dialog" aria-label="Menu de navigation mobile">
