  ...
  <section class="py-24 px-6 bg-white dark:bg-[#0A0D1A]" id="faq-section">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-center mb-16">
        <span class="inline-flex items-center gap-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 text-[11px] font-extrabold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4">
      ...
      <div class="space-y-4">
        <div v-for="item in faqs" :key="item.id" 
          class="border border-slate-200 dark:border-slate-800 rounded-[1.5rem] bg-white dark:bg-slate-900 overflow-hidden transition-all duration-300"
          ...
          <button @click="toggle(item.id)" 
            ...
            <span class="font-bold text-lg transition-colors" 
              :class="activeItem === item.id ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-800 dark:text-slate-200 group-hover:text-emerald-500'">
              {{ item.q }}
            </span>
            <div class="shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors"
              :class="activeItem === item.id ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-50 text-slate-400 dark:bg-slate-800'">
          ...
          <div class="grid transition-all duration-300 ease-in-out"
            ...
            <div class="overflow-hidden">
              <p class="px-6 pb-6 pt-0 text-slate-500 dark:text-slate-400 leading-relaxed">
                {{ item.a }}
              </p>
            </div>
          </div>
