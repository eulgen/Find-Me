...
import { Map, List, Grid, ShieldCheck, Sparkles, AlertTriangle } from 'lucide-vue-next'
...
const filteredAddresses = computed(() => {
  return addressesList.value.filter(addr => {
    ...
    const matchesCity = selectedCity.value === 'All' || addr.city === selectedCity.value
...
<template>
  <div class="space-y-6" id="global-citizen-map-dashboard">
    ...
    <div class="flex md:hidden border-2 border-[#1A237E] rounded-xl overflow-hidden shadow-sm" id="mobile-views-selector">
      <button 
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start" id="main-geographic-desktop-split">
    ...
    >
      <div class="bg-white border-4 border-rose-600 rounded-3xl p-6 max-w-sm w-full space-y-4 text-center shadow-2xl">
        <AlertTriangle class="w-12 h-12 text-rose-600 mx-auto animate-bounce" />
        <h3 class="text-lg font-black text-[#1A237E]">Confirmer le retrait ?</h3>
        <p class="text-xs text-[#1A237E]/75 font-normal leading-relaxed">
          Êtes-vous certain d'effacer définitivement cette adresse civique ? Cette action est irréversible.
        </p>
        <div class="flex justify-center space-x-3 pt-2">
          <button @click="showDeleteConfirm = false" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-black rounded-lg cursor-pointer">Annuler</button>
          <button @click="executeDeleteAddress" class="px-5 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black rounded-lg cursor-pointer">Supprimer</button>
        </div>
      </div>
    </div>
  </div>
