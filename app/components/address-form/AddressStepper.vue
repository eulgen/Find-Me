...
<template>
  <div class="bg-white/40 dark:bg-[#0A0D1A]/40 backdrop-blur-2xl rounded-[32px] shadow-[0_8px_32px_rgba(0,0,0,0.04)] border border-white/60 dark:border-slate-800/50 overflow-hidden relative group/container">
    ...
    <!-- HEADER & STEPPER -->
    <div class="bg-white/30 dark:bg-slate-900/30 backdrop-blur-md p-6 md:p-8 border-b border-white/60 dark:border-slate-800/50 relative z-10">
      ...
      <div class="flex items-center space-x-2 sm:space-x-4">
        ...
        <div class="flex items-center space-x-2">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all duration-500 z-10 relative" :class="
            currentStep >= 1 ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-emerald-500/30' : 'bg-white/50 dark:bg-slate-800/50 text-slate-400 border border-white/60 dark:border-slate-700/50'
          ">
            <Check v-if="currentStep > 1" class="w-5 h-5" />
            <span v-else>1</span>
          </div>
          <span :class="currentStep >= 1 ? 'font-bold text-slate-800 dark:text-white' : 'font-semibold text-slate-400 hidden sm:inline'">Localisation</span>
        </div>
        ...
        <div class="flex items-center space-x-2">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all duration-500 z-10 relative" :class="
            currentStep >= 2 ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-emerald-500/30' : 'bg-white/50 dark:bg-slate-800/50 text-slate-400 border border-white/60 dark:border-slate-700/50'
          ">
            <Check v-if="currentStep > 2" class="w-5 h-5" />
            <span v-else>2</span>
          </div>
          <span :class="currentStep >= 2 ? 'font-bold text-slate-800 dark:text-white' : 'font-semibold text-slate-400 hidden sm:inline'">Infos</span>
        </div>
        ...
        <div class="flex items-center space-x-2">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all duration-500 z-10 relative" :class="
            currentStep >= 3 ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-emerald-500/30' : 'bg-white/50 dark:bg-slate-800/50 text-slate-400 border border-white/60 dark:border-slate-700/50'
          ">
            3
          </div>
          <span :class="currentStep >= 3 ? 'font-bold text-slate-800 dark:text-white' : 'font-semibold text-slate-400 hidden sm:inline'">Photo</span>
        </div>
      </div>
    </div>
    ...
    <div class="p-6 md:p-8 min-h-[450px] relative z-10">
      ...
      <div v-if="currentStep === 1" class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
        ...
        <!-- THE MAP -->
        <div class="relative w-full h-96 md:h-[450px] bg-slate-100 dark:bg-slate-900 rounded-[32px] overflow-hidden border border-white/60 dark:border-slate-700/50 shadow-lg z-0">
        ...
        <div class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[32px] p-8 sm:p-10 border border-white/40 dark:border-slate-800/50 shadow-[0_8px_32px_rgba(0,0,0,0.06)] relative overflow-hidden">
          ...
          <template v-if="step1State.askQR">
            <div class="space-y-8 relative z-10">
              <div class="text-center">
                ...
                <h3 class="text-2xl font-black text-slate-800 dark:text-white">Avez-vous un voisin sur FindMe ?</h3>
                <p class="text-[15px] text-slate-500 dark:text-slate-400 mt-2 max-w-sm mx-auto font-medium">Récupérez instantanément son adresse pour créer la vôtre automatiquement.</p>
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <!-- Option 1: Scan Caméra -->
                <button @click="handleQRYes" class="group relative bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border-2 border-white/60 dark:border-slate-700/50 hover:border-emerald-500 rounded-[24px] p-6 text-left transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-emerald-500/10 ">
                  ...
                  <div class="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center mb-4 group-hover:bg-emerald-500/10 transition-colors shadow-inner">
                    <QrCode class="w-7 h-7 text-slate-500 dark:text-slate-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                  </div>
                  <h4 class="text-lg font-black text-slate-800 dark:text-white mb-2">1. Scanner le QR Code</h4>
                  <p class="text-[13px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed pr-8">
                ...
                <div class="relative">
                  ...
                  <button class="w-full h-full group relative bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border-2 border-white/60 dark:border-slate-700/50 hover:border-emerald-500 rounded-[24px] p-6 text-left transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-emerald-500/10 ">
                    <div class="absolute top-5 right-5 w-10 h-10 rounded-full bg-emerald-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md">
                      <ImageIcon class="w-5 h-5 text-white" />
                    </div>
                    <div class="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center mb-4 group-hover:bg-emerald-500/10 transition-colors shadow-inner">
                      <FileUp class="w-7 h-7 text-slate-500 dark:text-slate-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                    </div>
                    <h4 class="text-lg font-black text-slate-800 dark:text-white mb-2">2. Joindre une image</h4>
                    <p class="text-[13px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed pr-8">
              ...
              <!-- Action: Skip -->
              <div class="pt-6 text-center border-t border-white/40 dark:border-slate-800/50">
                <button @click="handleQRNo" class="text-[11px] font-black text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 uppercase tracking-widest transition-colors py-2.5 px-6 rounded-full bg-white/50 dark:bg-slate-800/50 border border-white/60 dark:border-slate-700/50 hover:shadow-md backdrop-blur-sm">
          ...
          <template v-else-if="step1State.askGeolocation">
            <div class="flex flex-col md:flex-row items-center gap-8 relative z-10">
              ...
              <div class="flex-1 text-center md:text-left">
                <h3 class="text-2xl font-black text-slate-800 dark:text-white mb-2">Utiliser la géolocalisation ?</h3>
                <p class="text-[15px] text-slate-500 dark:text-slate-400 font-medium">Pouvez-vous utiliser votre position actuelle pour remplir automatiquement le formulaire avec une haute précision ?</p>
              </div>
              <div class="flex items-center gap-4 w-full md:w-auto mt-6 md:mt-0">
                <ButtonUI @click="handleGeolocationNo" variant="outline" class="flex-1 md:w-32 bg-white/50 dark:bg-slate-800/50 border-white/60 shadow-sm text-slate-600">Non</ButtonUI>
          ...
          <template v-else-if="step1State.showQRScanner">
            <div class="space-y-4 relative z-10">
              <p class="text-sm font-bold text-center text-slate-500 dark:text-slate-400 flex items-center justify-center gap-2">
      ...
      <div v-if="currentStep === 2" class="space-y-8 animate-in slide-in-from-right-4 duration-500">
        <div class="p-5 bg-gradient-to-r from-orange-500/10 to-amber-500/5 border-l-4 border-orange-500 rounded-r-[24px] backdrop-blur-md shadow-sm">
          <p class="text-[15px] font-black text-slate-800 dark:text-white flex items-center gap-2">
            <Sparkles class="w-4 h-4 text-orange-500" /> Formulaire pré-rempli
          </p>
          <p class="text-[13px] font-medium text-slate-600 dark:text-slate-400 mt-1">Vérifiez les données géolocalisées et complétez les champs manquants avec précision.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-2">
          <div class="space-y-2">
            <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest pl-2">Pays</label>
            <input v-model="formState.country" type="text" disabled class="w-full bg-slate-100/50 dark:bg-slate-900/40 border border-white/40 dark:border-slate-800/50 text-slate-500 rounded-2xl px-5 py-3.5 font-bold shadow-inner opacity-70" />
          </div>
          <div class="space-y-2">
            <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest pl-2">Ville</label>
            <input v-model="formState.city" type="text" class="w-full bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 text-slate-900 dark:text-white rounded-2xl px-5 py-3.5 font-bold focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all shadow-inner" />
          </div>
          <div class="space-y-2">
          ...
          <div class="space-y-2">
            ...
            <input v-model="formState.houseNumber" type="text" placeholder="Ex: 28B" class="w-full bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm border border-emerald-500/40 text-emerald-600 dark:text-emerald-400 text-lg rounded-2xl px-5 py-3.5 font-black focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 outline-none transition-all shadow-inner placeholder:text-emerald-300" />
        ...
        <div class="flex justify-between items-center pt-6 border-t border-white/60 dark:border-slate-800/50">
          <ButtonUI @click="goBackToStep1" variant="outline" class="bg-white/50 dark:bg-slate-800/50 shadow-sm border-white/60 dark:border-slate-700/50 text-slate-600">
            Précédent
          </ButtonUI>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
            ...
            <div class="bg-emerald-500/5 backdrop-blur-md border border-emerald-500/20 rounded-[24px] p-6 relative overflow-hidden">
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="relative z-10">
                  <h4 class="text-[11px] font-black text-emerald-600 dark:text-emerald-400 mb-1.5 uppercase tracking-widest">Pourquoi une photo ?</h4>
                  <p class="text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                    Une photo claire de votre portail ou bâtiment est un repère visuel indispensable pour les livreurs et services d'urgence. Elle réduit de <span class="text-emerald-600 dark:text-emerald-400 font-bold">80%</span> le temps de recherche sur le terrain.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="md:col-span-3 flex flex-col">
            ...
            >
              ...
              <template v-else>
                <div class="w-20 h-20 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-white/40 shadow-xl rounded-full flex items-center justify-center mb-5 group-hover:bg-emerald-500 transition-all duration-500">
                  <Camera class="w-8 h-8 text-slate-500 group-hover:text-white transition-colors" />
                </div>
                <h3 class="text-xl font-black text-slate-800 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">Prendre une photo</h3>
            ...
            <div class="flex justify-between items-center mt-6 pt-6 border-t border-white/60 dark:border-slate-800/50">
              <ButtonUI @click="currentStep = 2" variant="outline" class="bg-white/50 dark:bg-slate-800/50 border-white/60 shadow-sm text-slate-600">
                Retour
              </ButtonUI>
    ...
    >
      <div v-if="showLimitModal" class="fixed inset-0 z-[100] bg-slate-900/40 dark:bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border border-white/60 dark:border-slate-800/50 rounded-[32px] p-8 max-w-sm w-full text-center shadow-[0_16px_64px_rgba(0,0,0,0.1)] relative overflow-hidden">
          ...
          <h3 class="text-2xl font-black text-slate-800 dark:text-white mb-3 tracking-tight relative z-10">Limite atteinte</h3>
          <p class="text-[14px] font-medium text-slate-500 dark:text-slate-400 leading-relaxed mb-8 relative z-10">
